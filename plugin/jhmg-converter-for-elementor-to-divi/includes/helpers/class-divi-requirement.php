 ...
 *
 * Neither plugin checked for Divi before. This is the check.
 */
...
class DiviRequirement {
     ...
     *
     * Divi 5 is the format boundary: 5.0.0 is the first release whose builder
     * reads `<!-- wp:divi/* -->` blocks and honours `_et_pb_use_divi_5`, and
     * every Divi 4 release ignores both.
     *
     * Divi's own public-alpha builds are not excluded by this floor. PHP reads
     * `5.0.0-public-alpha.18.2` as ABOVE `5.0.0`, because "public" is not one of
     * the qualifiers version_compare() ranks below a release — verified, not
     * assumed. Anyone still on such a build keeps working.
     */
    const MINIMUM_DIVI_VERSION = '5.0.0';

    /** Set at activation so the first admin page load can explain a failed install. */
    const ACTIVATION_NOTICE_OPTION = 'edc_divi_requirement_failed';
     ...
     * plugins — at `plugins_loaded` this returns null on a perfectly healthy
     * Divi site, which is why every caller here runs on an admin hook instead.
     */
    public static function detected_version(): ?string {
        ...
        // The Divi Builder plugin defines the constants above; the Divi theme
        // also does, but a child theme that has not booted its parent yet may
        // not have. Fall back to the theme header.
    ...
    public static function message(): string {
        ...
        if ( $reason === 'missing' ) {
            return sprintf(
                ...
                __( 'Elementor to Divi 5 Converter is inactive: Divi %s or newer is required, and no Divi installation was found. Converted pages would be saved in a format nothing on this site can render.', 'jhmg-converter-for-elementor-to-divi' ),
        ...
        if ( $reason === 'too_old' ) {
            return sprintf(
                ...
                __( 'Elementor to Divi 5 Converter is inactive: this site runs Divi %1$s, and the converter writes Divi 5 block content that requires Divi %2$s or newer. Converting now would produce pages that render blank.', 'jhmg-converter-for-elementor-to-divi' ),
    ...
    /**
     * Activation-time check. Activation runs inside an admin request, so the
     * theme is loaded and the version is readable here.
     *
     * The plugin is left active deliberately: deactivating it would hide the
     * explanation along with the plugin, and someone installing this before
     * installing Divi is doing the steps in a reasonable order. The flag makes
     * the next admin load say so.
     */
