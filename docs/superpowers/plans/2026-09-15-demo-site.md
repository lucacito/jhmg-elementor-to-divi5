...
The converter turns EAEL's filterable gallery into a plain Divi gallery: the images carry over, the filter buttons and item names do not. So the Spaces page's must-survive list checks image files for the gallery, not item names. Memberships' pricing tables have no subtitle, because the converter drops `eael_pricing_table_sub_title` (recorded in `docs/known-issues.md`).
...
Three converter bugs shape these pages (all recorded in `docs/known-issues.md`, not fixed in this work):
- EAEL's progress bar converts its percentage to `Array`, so About shows its member mix with core `counter` widgets instead.
- ElementsKit's heading converter drops the subtitle, so About's heading has none.
